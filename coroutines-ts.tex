...
\begin{frame}{Closer look into built-in coroutines}
  \alert{include in the view also boost.fiber for comparison}
  \begin{itemize}[<+- |alert@+>]
...
\begin{frame}{tasks and events excercise}
  \centerline{Write an application, that will at the same time (almost)}
  ...
  \begin{itemize}
    \item Read large content from a file
    \item Display dots every second
  \end{itemize}
...
\begin{frame}{promise\_type}
  \begin{itemize}[<+- |alert@+>]
    ...
    \begin{itemize}[<+- |alert@+>]
      ...
      \item it can be defined as (CHECK IT) \code{std::coroutine\_traits<returned\_type>::promise\_type}
    \end{itemize}
